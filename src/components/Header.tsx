...
export function Header() {
  ...
  const { siteName, categories, bannerText, isBannerActive, currentUser } = useSite();
  ...
  const showBanner = isBannerActive && showBannerLocal;
  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 inset-x-0 z-30 transition-colors duration-300 flex flex-col ${
        isScrolled ? 'bg-[#F9F7F2]/90 backdrop-blur-md border-b border-[#1A1A1A]/10' : 'bg-[#F9F7F2] border-b border-[#1A1A1A]/10'
      }`}
    >
      {showBanner && (
        <div className="w-full bg-[#1A1A1A] text-[#F9F7F2] py-2 px-6 flex items-center justify-center relative">
          <p className="font-sans text-[10px] uppercase tracking-widest font-bold text-center">
            {bannerText}
          </p>
          <button 
            onClick={() => {
      ...
      <div className="max-w-7xl mx-auto px-6 h-20 w-full flex items-center justify-between">
        ...
        <div className="flex-1 lg:hidden">
          <button className="p-2 -ml-2 hover:bg-[#1A1A1A]/5 rounded-full transition-colors" aria-label="Menu" onClick={() => setIsMenuOpen(true)}>
            <Menu className="w-5 h-5 text-[#1A1A1A]" />
          </button>
        </div>

        {/* Desktop Nav */}
        <nav className="flex-1 hidden lg:flex items-center gap-6 font-sans text-[10px] font-semibold tracking-[0.15em] uppercase text-[#1A1A1A]">
          {categories.slice(0, 3).map(cat => {
            ...
            return (
              <a 
                ...
                className={`transition-colors border-b ${isActive ? 'border-[#1A1A1A]' : 'text-[#1A1A1A]/80 border-transparent hover:border-[#1A1A1A] hover:text-[#1A1A1A]'}`}
              >
                {cat}
              </a>
            );
          ...
          <Link to="/live-sale" className="text-[#CDA185]/95 hover:text-[#CDA185] transition-colors font-bold">Live Sale ⚡</Link>
        </nav>
        ...
        <div className="flex-1 flex justify-end items-center gap-2 md:gap-6">
          ...
          >
            ...
            <span className="font-sans text-[11px] font-bold tracking-[0.1em] text-[#1A1A1A]">{coins}</span>
          </button>

          <button 
            className="flex p-2 hover:bg-[#1A1A1A]/5 rounded-full transition-colors cursor-pointer" 
            aria-label="Wishlist"
          ...
          <button 
            ...
            className="hidden md:flex p-2 hover:bg-[#1A1A1A]/5 rounded-full transition-colors cursor-pointer" 
          ...
          <button 
            className="flex items-center gap-2 hover:bg-[#1A1A1A]/5 p-2 rounded-full transition-colors cursor-pointer"
            aria-label="Cart"
            onClick={() => setIsCartOpen(true)}
          >
            <span className="text-[11px] font-bold tracking-tighter uppercase text-[#1A1A1A] hidden xl:block">CART ({cartCount})</span>
            <div className="w-5 h-5 md:w-4 md:h-4 bg-[#1A1A1A] rounded-full flex items-center justify-center relative">
              <ShoppingBag className="w-3 h-3 md:w-2 md:h-2 text-[#F9F7F2]" />
